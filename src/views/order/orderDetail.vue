              ...
              <div class="layui-form-item" v-else-if="domain.label === '产品名称'">
                ...
                <div class="layui-block">
                  <input v-model="domain.value" type="text" v-validate="'required'" :name="'name' + index" class="layui-input"
                         :class="{'input': true, 'is-danger': errors.has('name' + index)}" :disabled="disabled"/>
                ...
                for (let item in products[i]) {
                  for (let v in this.labelName) {
                    if (item === v) {
                      ...
                      if (item === 'operator') {
                        cc.push({
                          label: this.labelName[v],
                          placeholder: '',
                          property: v,
                          value: products[i][item].id
                        })
                      } else {
                        cc.push({
                          label: this.labelName[v],
      ...
      submitForm () {
        ...
        this.$validator.validateAll().then((result) => {
          if (result) {
            ...
            delete params.hibernateLazyInitializer
            // console.log(params)
